...
__device__ void cuda_arch_print_d()
{
#if __CUDA_ARCH__ >= 200
    printf("__CUDA_ARCH__ = %d\n", __CUDA_ARCH__);
#endif
}
__global__ void cuda_arch_print()
{
    cuda_arch_print_d();
}
...
__device__ void sgemm_device_w_boundary_check(float *A, float *B, float *C, int M, int N, int K)
{
    int bx = blockIdx.x;
    int by = blockIdx.y;
...
template <const int BM, const int BN, const int BK, const int TM, const int TN>
__device__ void sgemm_device_wo_boundary_check(float *A, float *B, float *C, int M, int N, int K)
{
    int bx = blockIdx.x;
    int by = blockIdx.y;
...
{
    sgemm_device_w_boundary_check<BM, BN, BK, TM, TN>(A, B, C, M, N, K);
}
...
__global__ void linear_w_boundary_check(float *input, float *weight, float *bias, float *output, int M, int N, int K)
{
    ...
    sgemm_device_w_boundary_check<BM, BN, BK, TM, TN>(input, weight, output, M, N, K);
...
__global__ void linear_wo_boundary_check(float *input, float *weight, float *bias, float *output, int M, int N, int K)
{
    ...
    int output_offset_x = blockIdx.x * BN;
    int output_offset_y = blockIdx.y * BM;
    sgemm_device_wo_boundary_check<BM, BN, BK, TM, TN>(input, weight, output, M, N, K);
    // point output to by * BM * N + bx * BN
    output = &output[OFFSET(output_offset_y, output_offset_x, N)];
    // point bias to bx * BN
    bias = &bias[output_offset_x];
...
__global__ void conv1d_k1_w_boundary_check(float *weight, float *input, float *bias, float *output, int M, int N, int K)
{
    ...
    sgemm_device_w_boundary_check<BM, BN, BK, TM, TN>(weight, input, output, M, N, K);
    // point output to by * BM * N + bx * BN
    output = &output[OFFSET(output_offset_y, output_offset_x, N)];
    // point bias to bx * BN
    ...
    int tile_col = threadIdx.x % BN;
    int tile_stride = thread_num / BN;
    // ! only works for tile_row=0
    float bias_tmp;
#pragma unroll
    for (int i = 0; i < BM; i += tile_stride)
    {
        bias_tmp = bias[tile_row + i];
        if (output_offset_x + tile_col < N && output_offset_y + tile_row + i < M)
        {
            output[(tile_row + i) * N + tile_col] += bias_tmp;
        }
    }
}
                                            ...
                                            int K)
{
    ...
    output = &output[OFFSET(output_offset_z, 0, M * N)];
    input = &input[OFFSET(output_offset_z, 0, K * N)];
    sgemm_device_wo_boundary_check<BM, BN, BK, TM, TN>(weight, input, output, M, N, K);
    // point output to by * BM * N + bx * BN
    output = &output[OFFSET(output_offset_y, output_offset_x, N)];
    // point bias to bx * BN
    bias = &bias[output_offset_y];
    ...
    cu::Tensor<T> forward(cu::Tensor<T> &input) override
    {
        ...
        if (input.is_cuda)
        {
            ...
            if (std::is_same<T, float>::value)
            {
                ...
                {
                    cu::kernel::linear_w_boundary_check<BM, BN, BK, TM, TN>
                        <<<gridDim, blockDim>>>(input.data, weight.data, bias.data, result.data, input.shape[0],
                                                weight.shape[1], input.shape[1]);
                }
            }
